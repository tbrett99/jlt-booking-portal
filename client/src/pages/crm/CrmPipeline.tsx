...
import { Plus, Search, Mail, Phone, Tag, ChevronRight, User } from "lucide-react";
...
export default function CrmPipeline() {
  ...
  const moveStage = trpc.crm.prospects.moveStage.useMutation({ onSuccess: () => { refetch(); setMoveDialog(null); setMoveNote(""); toast.success("Stage updated"); } });
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{(prospects as any[]).length} prospects total</p>
        </div>
                ...
                <div className="bg-muted/30 rounded-b-lg min-h-[200px] p-2 space-y-2 border border-t-0 border-border">
                  {cards.map((p: any) => (
                    <Card
                      ...
                      onClick={() => navigate(`/crm/prospects/${p.id}`)}
                    >
                      <div className="flex items-start justify-between gap-1">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            {p.uniqueAgentId && <p className="text-[10px] text-muted-foreground font-mono">{p.uniqueAgentId}</p>}
                          </div>
                        </div>
                      ...
                      <div className="mt-2 pt-2 border-t border-border flex justify-between items-center">
                        ...
                        <Button
                          ...
                          onClick={(e) => { e.stopPropagation(); setMoveDialog({ prospectId: p.id, name: `${p.firstName} ${p.lastName}`, currentStage: p.stage }); setNewStage(p.stage); }}
                        >
                          Move
                        </Button>
                      </div>
