...
function TaskFormDialog({
  ...
  const [assigneeId, setAssigneeId] = useState<string>(initial?.assigneeId ? String(initial.assigneeId) : "unassigned");
  const [dueDate, setDueDate] = useState(initial?.dueDate ? format(new Date(initial.dueDate), "yyyy-MM-dd") : "");
  const [linkedType, setLinkedType] = useState<LinkedType>(initial?.linkedType ?? "none");
  const [linkedId, setLinkedId] = useState(initial?.linkedId ? String(initial.linkedId) : "");
  ...
  function handleSubmit() {
    ...
    const payload = {
      ...
      linkedType,
      linkedId: linkedType !== "none" && linkedId ? Number(linkedId) : undefined,
    };
    if (isEdit && initial) {
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? "Update the task details below." : "Create a new task and optionally assign it to an admin."}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={linkedType} onValueChange={(v) => { setLinkedType(v as LinkedType); setLinkedId(""); }}>
                <SelectTrigger className="h-9">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {linkedType !== "none" && (
              <div>
                <label className="text-sm font-medium mb-1 block">{LINKED_TYPE_LABELS[linkedType]} ID</label>
                <Input
                  type="number"
                  placeholder="e.g. 42"
                  value={linkedId}
